 ...
 * 
 * @author Akshata
 * <p>This class is used as a controller
 *
 */
@Controller
public class FlightController {
	...
	public String insertBookingDetails(@RequestParam("sid") Integer sid, @RequestParam("psgnr") Integer seats,
		...
		//BookingDetails bd=dao.getPnrId(custid);
		int pnrid=ser.updateSeat(sid, seats, custid);
		BookingDetails bd=dao.getDetails(pnrid);
		//int pnrno=bd.getPnrId();
		logger.debug("pnrno="+pnrid);
		//model.addAttribute("pnrno", dao.getPnrId(custid));
		// model.addAttribute("price", dao.getFare(sid));
		model.addAttribute("details", bd);
		logger.debug("details=" +bd.getPassengers());
